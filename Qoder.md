...
| 入口 / 服务 / 页面 | 根 `main.py`（本机）或 `passenger_wsgi.py`（公网）→ `api/app.py`（模块注册表，公网在此剔除私有路由）→ `api/core.py`（配置/日志/缓存/`dispatch()`+`wsgi_app()`/静态）→ 四个取数模块（`btc` / `liquidity` / `crash` / `allocation`）→ `staic/*.html`（公共层 `staic/common.css`、`staic/common.js` 五页共享，导航表是 `common.js` 的 `PAGES`） |
